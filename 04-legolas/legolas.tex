...
Therefore, once the domain is chosen, the approximation of a state variable $x^i$ (with $i$ the state vector index) for a domain divided into $N$ subdomains (nodes) is given by
...
\end{figure}
The Hermite and quadratic basis functions themselves, hereafter denoted by $C_j$ and $Q_j$, respectively, are given by
...
\begin{align}
  ...
  C^1_j(x) &=
  \begin{cases}
    \left(\dfrac{x - x_{j-1}}{x_j - x_{j-1}}\right)^2\left(3 - 2\dfrac{x - x_{j-1}}{x_j - x_{j-1}}\right) 	\qquad &\text{for}~ x_{j-1} \leq x \leq x_j,	\\
    \left(\dfrac{x_{j+1} - x}{x_{j+1} - x_j}\right)^2\left(3 - 2\dfrac{x_{j+1} - x}{x_{j+1} - x_j}\right) 	\qquad &\text{for}~ x_j \leq x \leq x_{j+1},	\\
  ...
  C^2_j(x) &=
  \begin{cases}
    \left(x - x_j\right)\left(\dfrac{x - x_{j-1}}{x_j - x_{j-1}}\right)^2	\qquad &\text{for}~ x_{j-1} \leq x \leq x_j,	\\
    \left(x - x_j\right)\left(\dfrac{x_{j+1} - x}{x_{j+1} - x_j}\right)^2	\qquad &\text{for}~ x_j \leq x \leq x_{j+1},	\\
    0	\qquad &\text{elsewhere},
  \end{cases} \label{eq: basisfunction_C2}
\end{align}
\endgroup
as in, for example, \citet{kerner1998} and \citet{book_MHD}, and are shown graphically along with their derivatives in Figure \ref{fig: basisfunctions}. Because we have two basis functions per subdomain, this allows for the approximation of both the original variable and its derivative by differentiation of Equation \eqref{eq: linear_combo}.

\subsubsection{Variational analysis and Galerkin form}
Before proceeding with our analysis we hold a small mathematical interlude on variational analysis, and how writing the problem in weak Galerkin form is quite convenient in our current formalism.
A thorough discussion is given in \citet[Chapter 15]{book_MHD}; here we follow a similar reasoning and highlight the key ideas behind the weak formalism. Consider a given inhomogeneous differential equation of the form
\begin{equation}
  -\bigl[p(u_1)R(u_1)'\bigr]' + q(u_1)R(u_1) = f(u_1),
\end{equation}
on an interval $u_1 \in [0, 1]$ for given (known) functions $p(u_1), q(u_1)$ and $f(u_1)$, where the prime denotes the derivative with respect to $u_1$ and we assume the first and second order derivatives of $R(u_1)$ exist on $[0, 1]$.
If we were to solve the above equation on a domain discretised in $N$ subdomains, we would obtain an approximate solution $\hat{R}(u_1)$. The quality of this solution can be estimated by calculating the \emph{residual} $\hat{r}_s^N(u_1)$, that is, the difference between the analytical and approximate solution, which can be quantified without actually knowing the actual solution by straightforwardly filling in $\hat{R}$ into the original differential equation:
\begin{equation} \label{eq: residual}
  \hat{r}_s^N(u_1) \equiv -\bigl[p(u_1)\hat{R}(u_1)'\bigr]' + q(u_1)\hat{R}(u_1) - f(u_1).
\end{equation}
This residual should be as small as possible, and the solution $\hat{R}(u_1)$ is said to converge if the residual vanishes in the limit $N \rightarrow \infty$ everywhere on the domain; or, in other words, when the approximate solution is equal to the exact solution.

In a finite element analysis this solution $\hat{R}(u_1)$ is given by a linear combination of basis functions as given by Equation \eqref{eq: linear_combo}, but to keep things simple here we will stick with a single (linear) basis function $h_i$ such that
\begin{equation} \label{eq: linear_combo_R}
  R(u_1) \approx \hat{R}(u_1) = \sum_{i = 0}^N r_i h_i(u_1),
\end{equation}
where now $r_i$ are the coefficients in the linear combination. Writing the original differential equation in its integral form
\begin{equation}
  \int_0^1\Bigl(-\bigl[p(u_1)R(u_1)'\bigr]' + q(u_1)R(u_1) - f(u_1)\Bigr)du_1 = 0,
\end{equation}
then we can obtain a linear system of algebraic equations for the coefficients $\{r_i\}$ by requiring that the weighted integrals of the residual \eqref{eq: residual} vanish for the approximate solution, that is,
\begin{equation} \label{eq: galerkin_form}
  \int_0^1h_l\Biggl(-\bigl[p(u_1)\hat{R}(u_1)'\bigr]' + q(u_1)\hat{R}(u_1) - f(u_1)\Biggr) = 0,
\end{equation}
where the weight functions $w_l$ are taken equal to the basis functions; the function $\hat{R}$ is given by Equation \eqref{eq: linear_combo_R}. The above expression is called the \emph{Galerkin method}, where the original differential equation is now rewritten as a linear system of $N + 1$ algebraic equations for $N + 1$ unknowns $\{R_i\}_{i = 0, 1, \ldots N}$.

\subsubsection{The MHD equations in weak Galerkin form}
Using the Galerkin form we just discussed we can turn our original set of equations \eqref{eq: full_continuity}--\eqref{eq: full_induction3} algebraic in nature, by writing the eigenvalue problem as a set of integral equations. Each of the eight equations is multiplied by an appropriate element of the chosen basis, denoted by $h_j$, and integrated over the relevant domain. Mathematically this can be written as
\begin{equation}
  \int_\Omega\Bigl(\amat \statevec - \omega \bmat \statevec\Bigr)h_j du_1 = 0,
\end{equation}
which will yield an equation similar to \eqref{eq: galerkin_form}. However, the matrix $\amat$ contains second-order derivatives with respect to $u_1$. With the problem written in the above form we can reduce these to derivatives of first order, thereby simplifying the integrals. This is achieved by performing integration by parts, which introduces additional surface terms that have to be evaluated at the boundaries. This integration by parts allows for solutions that are continuously differentiable to a lower degree (and hence less ``smooth''), and this is called the \emph{weak Galerkin formulation} of the problem. The surface terms that are introduced by integration by parts can in turn be exploited to enforce boundary conditions. Because there are eight unknowns in our eigenvalue system, together with two basis functions and $N$ subdomains, this implies that the final matrix eigenvalue problem will have $16 N$ equations, resulting in a $16N \times 16N$ matrix. By extension, it becomes clear that using basis functions of even higher order will increase the size of this eigensystem considerably.
