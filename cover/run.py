...
def initapplications():
    ...
    apps.pdflatex = App(
        "pdflatex",
        "-interaction=nonstopmode -synctex=1 -shell-escape {basename}",
        verbose,
    )
    apps.bibtex = App("bibtex", "--min-crossref=100 {basename}", verbose)
    apps.biber = App("biber", "{basename}", verbose)
    apps.glossary = App(
        "makeindex", "{basename}.glo -s {basename}.ist -o {basename}.gls", verbose
    )
    apps.nomenclature = App(
        "makeindex", "{basename}.nlo -s nomencl.ist -o {basename}.nls", verbose
    )
...
settings.chapters = [
    name.replace(".tex", "") for name in glob.glob("chapters/**/*.tex")
]
settings.cleanfiles = " ".join(
    [
        base + ext
        for ext in settings.cleanext
        for base in [settings.basename] + settings.chapters
    ]
)
settings.pdffile = settings.basename + ".pdf"

apps = create(
    "pdflatex", "bibtex", "biber", "glossary", "nomenclature", "pdfviewer", "remove"
)
...
def cover():
    ...
    if verbose > 0:
        ...
        print(usersettings)
    extra_usersettings = ",".join(
        [""] + ["{}={}".format(k, v) for k, v in usersettings.items()]
    )

    with open("cover.tex", "w") as cf:
        cf.write(
            """% Cover.tex
\\documentclass[cam,final,cover{}]{{adsphd}}""".format(
                extra_usersettings
            )
        )
        cf.write(
...
def testBiblatex():
        ...
        for line in f:
            if pattern.search(line) != None:
                ...
                if not settings.usebiblatex:
                    print(
                        "WARNING: It appears you are using biblatex while this setting in run.py is set to false.\n"
                    )
                    allok = False
    ...
    if not isusingbiblatex and settings.usebiblatex:
        print(
            "WARNING: It appears you are not using biblatex while this setting in run.py is set to true.\n"
        )
...
definition
    ...
    parser.add_argument(
        "--targets", "-T", action="store_true", help="Print available targets"
    )
    parser.add_argument(
        "--dry",
        "-d",
        action="store_true",
        help="Dry run to see commands without executing them",
    )
...
if __name__ == "__main__":
    main()
    for ext in [".log", ".out", ".synctex.gz", ".tex", ".aux"]:
        os.remove(f"cover{ext}")
    shutil.move(f"cover.pdf", "cover/cover.pdf")
